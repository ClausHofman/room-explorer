...
def load_game(filename):
    ...
    for room_data in game_state.get('rooms', []):
        ...
        room.creatures = {}  # Initialize creatures dictionary
    ...
    for room_data in game_state.get('rooms', []):
        room = room_dict[room_data['name']]
        creatures_data = room_data.get('creatures', [])
        for creature_dict in creatures_data:
            for creature_key, creature_data in creature_dict.items():
                if creature_key == str(Creature):
                    creature = Creature(
                        creature_data['name'],
                    ...
                    room.creatures[creature_data['name']] = creature
...
def serialize_rooms(rooms):
    ...
    for room in rooms:
        ...
        serialized_room = {
            ...
            'creatures': [serialize_creature(creature) for creature in room.creatures.values()],
            'inventory': serialize_inventory(room.inventory)  # Serialize room inventory
        }
...
def serialize_creature(creature):
    return {
        str(Creature): {
            ...
            'current_room': creature.current_room.name,
...
class Room:
    ...
    def serialize(self):
        return {
            ...
            'exits': {direction: neighbor.name for direction, neighbor in self.exits.items()},
            'objects': [obj.__dict__ for obj in self.objects],
            'creatures': [self.serialize_creature(creature) for creature in self.creatures.values()],
            'inventory': serialize_inventory(self.inventory)  # Ensure inventory is serialized correctly
        }
    ...
    @staticmethod
    def deserialize(data, room_dict):
        room = Room(data['name'], data['description'])
        room.exits = {direction: room_dict[neighbor_name] for direction, neighbor_name in data['exits'].items()}
        room.objects = [GameObject(obj['name'], obj['description']) for obj in data.get('objects', [])]
        room.inventory = deserialize_inventory(data.get('inventory', {}))

        for creature_data in data.get('creatures', []):
            for creature_key, creature_info in creature_data.items():
                if creature_key == str(Creature):
                    creature = Creature(
                        creature_info['name'],
                        creature_info['description'],
                        room,
                        creature_info['move_interval']
                    )
                    creature.wandering = creature_info['wandering']
                    room.creatures[creature_info['name']] = creature
...
def main():
    ...
    # Create a room and a creature
    hall = Room("Hall", "You are in a grand hall with a doorway leading to north.")
    new_creature = player.create_creature("test creature", "test creature", 10)
    new_creature2 = player.create_creature("test creature 2", "test creature 2", 10)

    # Serialize the room
    # serialized_room = hall.serialize()
    # print("Serialized Room:")
    # print(json.dumps(serialized_room, indent=2))

    # # Deserialize the room
    # room_dict = {serialized_room['name']: hall}
    # deserialized_room = Room.deserialize(serialized_room, room_dict)
