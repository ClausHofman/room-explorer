...
def save_game(player, filename):
    ...
    print(f"Pringting game state: {game_state}")
    print(f"Pringting game rooms: {serialized_rooms}")
...
def user_input_thread(player, quit_flag):
    ...
    while not quit_flag.is_set():
        try:
            user_input = prompt("Enter a command: look, create, remove, object_names, create_creature, move_creature, remove_creature, create_item, pick_up, drop, inventory, save, load, quit): ")
            # Check if the input is empty or contains only whitespace
            if not user_input.strip():
                print("Error, please try again. Available commands: look, create, remove, object_names, create_creature, move_creature, pick_up, create_item, drop, inventory, save, load, quit.")
